...
//*****************************************************************
//
// !!! Note: This sample requires an INTEL I210 Adapter !!!
//
//*****************************************************************
...
void static AppTask(PVOID)
{
	...
	{
		//Check station name
		if (__PnioCheckStationName(pStation, "station5"))
		{

	//*****************************************************
	SYSTEM_SEQ_CAPTURE("APP", __LINE__, *((PULONG64)&__pUserList[1].OutputFrameData[0]), FALSE);
	SYSTEM_SEQ_CAPTURE("APP", __LINE__, *((PULONG64)&__pUserList[1].OutputFrameData[8]), FALSE);
	SYSTEM_SEQ_CAPTURE("APP", __LINE__, *((PULONG64)&__pUserList[1].OutputFrameData[16]), FALSE);
	SYSTEM_SEQ_CAPTURE("APP", __LINE__, *((PULONG64)&__pUserList[1].OutputFrameData[24]), FALSE);
//	SYSTEM_SEQ_CAPTURE("APP", __LINE__, *((PULONG64)&__pUserList[1].OutputFrameData[32]), TRUE);
	//*****************************************************
...
//	sprintf(PnioParams.szStationFile, "c:\\pnt\\StationList (F5868).par");
//	sprintf(PnioParams.szStationFile, "c:\\pnt\\StationList (CN8032).par");
//	sprintf(PnioParams.szStationFile, "c:\\pnt\\StationList (CN8032_F5868).par");
	sprintf(PnioParams.szStationFile, "c:\\pnt\\StationList (CN8032_F5868_S5868).par");
	//*********************************************
 	...
 	if (ERROR_SUCCESS == Sha64PnioCreate(&PnioParams))
	{
		...
		while (!kbhit())
		{
			...
			printf("InData:[%08x] [%08x] [%08x], UpdateCnt:%i [%c]\r",					
					__InData[0], __InData[1], __InData[2], __UpdateCnt, ActChar[++c%4]);
