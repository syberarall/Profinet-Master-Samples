...
//	sprintf(PnioParams.szStationFile, "c:\\pnt_irt\\StationList (C65M).par");
	sprintf(PnioParams.szStationFile, "c:\\pnt_irt\\StationList (C582M).par");
	//*********************************************
	...
	if (ERROR_SUCCESS == Sha64PnioCreate(&PnioParams))
	{
		...
		while (!kbhit())
		{
			...
			printf("Pos:[%08x], UpdateCnt:%i [%c]\r",
					__Position,	__UpdateCnt, ActChar[++c%4]);
