...
void static AppTask(PVOID)
{
	...
	//**************************************
		...
		if (__PnioCheckStationName(pStation, "station1"))
		{
			...
			//Phoenix ILB PN24 DI16 DIO16 Modul
			//Set output data to   Slot1 (2 Byte Output)
			//Get input  data from Slot2 (2 Byte Input)
				...
				(pOutData))
			{
				...
				pOutData[0] = __InData[0] = pInData[0];
				pOutData[1] = __InData[1] = pInData[1];
...
void main(void)
{
	...
	PnioParams.EthParams.drv_num = 1; //0 : for first NIC, 1 : for sencond NIC
	...
	sprintf(PnioParams.szStationFile, "StationList (ILB PN24).par");
	//*****************************
	...
	if (ERROR_SUCCESS == Sha64PnioCreate(&PnioParams))
	{
		...
		while (!kbhit())
		{
			...
			printf("InData:[%02x][%02x], UpdateCnt:%i [%c]\r",					
					__InData[0], __InData[1], __UpdateCnt, ActChar[++c%4]);
