...
#define SEND_CYCLE_TIME		8000
#define REALTIME_PERIOD		 150
...
void static AppTask(PVOID)
{
	...
	//**************************************
		...
		//*****************************************************
		SYSTEM_SEQ_CAPTURE("APP", __LINE__, 0, FALSE);
		//*****************************************************
...
//	sprintf(PnioParams.szStationFile, "StationList (3xF5868).par");		//Use SEND_CLOCKED with SendMode = 1 (registry)
	sprintf(PnioParams.szStationFile, "StationList (20xF5868).par");	//Use SEND_CLOCKED with SendMode = 1 (registry)
	//*****************************
	...
	if (ERROR_SUCCESS == Sha64PnioCreate(&PnioParams))
	{
		...
		{
			//Enable station
			if (ERROR_SUCCESS != Pnio64EnableStation(&__pUserList[i]))
			{
				printf("Enable station failed, press any key ...\n");
