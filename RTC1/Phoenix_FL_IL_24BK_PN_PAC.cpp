...
void static AppTask(PVOID)
{
	...
	//**************************************
		...
		//Check station name
		if (__PnioCheckStationName(__pStation, "station1"))
		{
			PUCHAR pInData;
			PUCHAR pOutData;
			...
			PNIO_GET_OUTPUT_DATAPTR(__pStation, 2, 0, &pOutData);
			PNIO_GET_INPUT_DATAPTR (__pStation, 3, 0, &pInData);

			if ((pInData) &&
				(pOutData))
			{
				//Set input data to output data
				pOutData[0] = __InData = pInData[0];
...
void main(void)
{
	...
	if (ERROR_SUCCESS == Sha64PnioCreate(&PnioParams))
	{
		...
		while (!kbhit())
		{
			...
			printf("InData:[%02x], UpdateCnt:%i [%c]\r",					
					__InData, __UpdateCnt, ActChar[++c%4]);
